...
const ViewBandejaDocumentosDictaminadoresUnoSecPitTres = () => {
...
const eliminar=async(id)=>{
  ...
  const agregar= async(e)=>{
   ...
   try {        
        ...
        if(await db.collection('endoresoluciondictaminadorunosectres').add(nuevo)){
...
const [revisadouno, setRevisadouno] = useState('revisado')
    const editar = async (id) => {
        
        try {
            
          await db.collection('endodictaminadoressecpittres').doc(id).update({
            activadordictauno: revisadouno
          })
          const data= await db.collection('endodictaminadoressecpittres').orderBy('fecha','desc').get()
          const arrayData=  data.docs.map(doc=>({id:doc.id,...doc.data()}))
          setDocus(arrayData)  
    ...
    <div>
      <div className="container text-enter bg-info mt-5">
        ...
        <div className="row">
            ...
            {
                usuario.rol==='dictaminadoruno' ?  <h2>Documentos recibidos dictaminador A</h2>:null||
                usuario.rol==='admin' ?  <h2>Documentos recibidos </h2>:null
                ...
                <div className="table-responsive-md">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td className="table-success">{item.activadordictauno}</td>
...
export default ViewBandejaDocumentosDictaminadoresUnoSecPitTres