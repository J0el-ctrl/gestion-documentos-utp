...
const ViewBandejaInformesDictaminadoresSecPitDos = () => {
  ...
  useEffect(() =>  {
    const obtenerDatos=async()=>{
        try {
          ...
          const data= await db.collection('endoresoluciondictaminadorunosecdos').get()
  ...
  useEffect(() =>  {
    const obtenerDatos=async()=>{
        try {
            ...
            //app debe ser igual a lo que se exporta                
            //aqui la respuesta se esta guardando en data
          const data= await db.collection('endoresoluciondictaminadordossecdos').get()
            //{id:doc.id,...doc.data()} objetos que trae de la base de datos
            const arrayData=  data.docs.map(doc=>({id:doc.id,...doc.data()}))
            // console.log(arrayData);
  ...
  useEffect(() =>  {
    const obtenerDatos=async()=>{
        try {
            ...
            //app debe ser igual a lo que se exporta                
            //aqui la respuesta se esta guardando en data
          const data= await db.collection('endoresoluciondictaminadortressecdos').get()
            //{id:doc.id,...doc.data()} objetos que trae de la base de datos
            const arrayData=  data.docs.map(doc=>({id:doc.id,...doc.data()}))
            // console.log(arrayData);
...
const eliminar=async(id)=>{
  try {
     await  db.collection('endoresoluciondictaminadorunosecdos').doc(id).delete()
...
const eliminardos=async(id)=>{
  try {
     await  db.collection('endoresoluciondictaminadordossecdos').doc(id).delete()
...
const eliminartres=async(id)=>{
  try {
     await  db.collection('endoresoluciondictaminadortressecdos').doc(id).delete()
  ...
  const agregar=async(e)=>{
    ...
    try {
        const nuevo={
          ...
          if(  await db.collection('endoinforados').add(nuevo)){
...
// MAPEAR CANTIDAD DE DOCUMENTOS INGRESADOS POR DOCENTE
...
export default ViewBandejaInformesDictaminadoresSecPitDos