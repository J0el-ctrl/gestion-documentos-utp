...
const ViewBandejaInformesDictaminadores = () => {
  ...
  useEffect(() =>  {
    const obtenerDatos=async()=>{
        try {
          ...
          const data= await db.collection('endoresoluciondictaminadoruno').get()
  ...
  useEffect(() =>  {
    const obtenerDatos=async()=>{
        try {
            ...
            //app debe ser igual a lo que se exporta                
            //aqui la respuesta se esta guardando en data
          const data= await db.collection('endoresoluciondictaminadordos').get()
            //{id:doc.id,...doc.data()} objetos que trae de la base de datos
            const arrayData=  data.docs.map(doc=>({id:doc.id,...doc.data()}))
            // console.log(arrayData);
  ...
  useEffect(() =>  {
    const obtenerDatos=async()=>{
        try {
            ...
            //app debe ser igual a lo que se exporta                
            //aqui la respuesta se esta guardando en data
          const data= await db.collection('endoresoluciondictaminadortres').get()
            //{id:doc.id,...doc.data()} objetos que trae de la base de datos
            const arrayData=  data.docs.map(doc=>({id:doc.id,...doc.data()}))
            // console.log(arrayData);
...
const eliminar=async(id)=>{
  try {
     await  db.collection('endoresoluciondictaminadoruno').doc(id).delete()
...
const eliminardos=async(id)=>{
  try {
     await  db.collection('endoresoluciondictaminadordos').doc(id).delete()
...
const eliminartres=async(id)=>{
  try {
     await  db.collection('endoresoluciondictaminadortres').doc(id).delete()
  ...
  const agregar=async(e)=>{
    ...
    try {
        const nuevo={
          ...
          if(  await db.collection('endoinforauno').add(nuevo)){
...
// MAPEAR CANTIDAD DE DOCUMENTOS QUE JALA DE DOCENTE INGRESNADO 
    ...
    <div className="row">
        ...
        <p>Documentos Procesados    : {docus.length}</p>
        {
         docucantidadjala.map(item=>(
                ...
                <p key={item.id}>Documentos Pendientes: {item.docucantidad}</p>  
...
export default ViewBandejaInformesDictaminadores