...
const ViewBandejaDocumentosDictaminadoresTresSecPitTres = () => {
  ...
  useEffect(() =>  {
    const obtenerDatos=async()=>{
        try {
            ...
            const data= await db.collection('endodictaminadoressecpittres').orderBy('fecha','desc').get()
...
const eliminar=async(id)=>{
  try {
     await  db.collection('endodictaminadoressecpittres').doc(id).delete()
  ...
  const agregar= async(e)=>{
   ...
   try {        
        ...
        if(await db.collection('endoresoluciondictaminadortressectres').add(nuevo)){
...
const [revisadotres, setRevisadotres] = useState('revisado')
const editar = async (id) => {
        
    try {
        
      await db.collection('endodictaminadoressecpittres').doc(id).update({
        activadordictatres: revisadotres
      })
      const data= await db.collection('endodictaminadoressecpittres').orderBy('fecha','desc').get()
      const arrayData=  data.docs.map(doc=>({id:doc.id,...doc.data()}))
      setDocus(arrayData)  
    ...
    <div>
       <div className="container text-enter bg-warning mt-5">
        ...
        <div className="row">
            ...
            {
                usuario.rol==='dictaminadortres' ?  <h2>Documentos recibidos dictaminador C</h2>:null||
                usuario.rol==='admin' ?  <h2>Documentos recibidos DC </h2>:null
                ...
                <div className="table-responsive-md">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td className="table-success">{item.activadordictatres}</td>
...
export default ViewBandejaDocumentosDictaminadoresTresSecPitTres